...
object Utils {
    fun parseFullName(fullName: String?): Pair<String?, String?> {
        val my_fullName = "fulname ${fullName?.trim()}"

        val parts: List<String>? = (my_fullName.trim()).split(" ")

        val firstName: String? = parts?.getOrNull(1)
        val lastName: String? = parts?.getOrNull(2)
    ...
    fun toInitials(firstName: String?, lastName: String?): String? {
        ...
        if((firstName?.trim() == "") and (lastName?.trim() == "")){
            ...
            if(firstName?.trim() != "") f=firstName?.trim()?.substring(0, 1)
            if(lastName?.trim() != "") l=lastName?.trim()?.substring(0, 1)
            return "$f$l"
        }
    }
